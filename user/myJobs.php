...
$sql = "
    SELECT 
        ...
        jobs.budget, 
        jobs.status, 
        COALESCE(jobs.cancelled_at, 'Not Available') AS cancelled_at,
        COALESCE(jobs.cancelled_by, 0) AS cancelled_by,
        COALESCE(jobs.chosen_worker_id, 0) AS chosen_worker_id,
        employer.first_name AS employer_first_name,
        employer.last_name AS employer_last_name,
        COALESCE(canceller.first_name, 'Unknown') AS canceller_first_name,
        COALESCE(canceller.last_name, '') AS canceller_last_name,
        COALESCE(worker.first_name, 'No Worker') AS worker_first_name,
        COALESCE(worker.last_name, '') AS worker_last_name
    FROM jobs
...
// Now close the connection after all queries are executed
$conn->close();
?>
...
<script>
    function jobDashboard() {
        return {
            ...
            ongoingJobs: [],
            in_progress_jobs: [],
            cancelledJobs: [], // New: Stores cancelled jobs
            showCancelModal: false,
            showSuccessModal: false, // New: Success modal for cancellation
            jobIdToCancel: null,

            async init() {
                await this.loadInProgressJobs();
                await this.loadCompletedJobs();
                await this.fetchCancelledJobs(); // New: Load cancelled jobs on page load
            },
            ...
            async fetchCancelledJobs() {
                try {
                    let response = await fetch('getCancelledJobs.php'); // Fetch cancelled jobs
                    let data = await response.json();
                    this.cancelledJobs = data;
                } catch (error) {
                ...
                fetch('cancelJob.php', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            this.fetchCancelledJobs(); // Update the Cancelled tab
                        } else {
            ...
            confirmCancel() {
                console.log("Job ID being sent:", this.jobIdToCancel); // Debugging
                if (!this.jobIdToCancel) {
                ...
                fetch('cancelJob.php', {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            this.showCancelModal = false; // Hide modal
                            this.showSuccessModal = true; // Show success modal
                            this.fetchCancelledJobs(); // Update cancelled jobs list
                        } else {
                            alert("Error cancelling job: " + data.error);
                        }
                    })
            ...
            cancelledJobs() {
                return {
                    ...
                    async init() {
                        try {
                            let response = await fetch('getCancelledJobs.php');
                            let data = await response.json();
                            this.cancelled_jobs = data;
                        } catch (error) {
                            console.error('Error fetching cancelled jobs:', error);
                        } finally {
                            this.loading = false;
...
<script>
    function jobTable() {
        return {
            ...
            imageModalSrc: "",
            showCompleteConfirmModal: false, // Modal for confirming completion
            showCancelConfirmModal: false, // Modal for confirming cancellation
            showCancelSuccessModal: false, // Fixed success modal variable
            ...
            async cancelJob(jobId) {
                console.log("Attempting to cancel job ID:", jobId); // ✅ Debugging
                try {
                    const response = await fetch('/Quickfix/public/api/cancel_in_progress_job.php', {
                        method: 'POST',
                        headers: {
                    ...
                    console.log("Raw Response:", text); // ✅ Debugging: Print raw API response
            ...
            // Open confirmation modal with selected job
            showCancelConfirmation(job) {
                this.selectedJob = job;
            ...
            async confirmCancel() {
                if (!this.selectedJob) {
                    console.error("Error: No job selected for cancellation.");
                    return;
                }

                await this.cancelJob(this.selectedJob.id);

                this.$nextTick(() => {
                    this.showCancelConfirmModal = false; // Close confirmation modal
                    this.showCancelSuccessModal = true; // Show success modal
                });
            },
            ...
            confirmCompletion(job) {
                // Set the selected job for completion confirmation
                this.selectedJob = job;
                this.showCompleteConfirmModal = true; // Show the completion confirmation modal
            },
    ...
    <div id="main-content" style="display:none;">
        <div class="w-full max-w-5xl mx-auto p-3 sm:p-6">
            ...
            <div class="flex flex-wrap space-x-2 sm:space-x-4 border-b pb-3 mb-4 sm:mb-6 overflow-x-auto">
                ...
                <button class="px-3 py-1 sm:px-4 sm:py-2 whitespace-nowrap text-sm sm:text-base"
                    :class="currentTab === 'cancelled' ? 'text-blue-700 font-semibold border-b-2 border-blue-700' : 'text-gray-500 hover:text-blue-700'"
            ...
            <div x-show="selectedJob && currentTab === 'open'" id="jobDetailsSection"
                ...
                <div class="border-t border-gray-200 pt-4 mt-4 flex flex-wrap justify-end gap-3 relative z-10">
                    <button @click="viewOffers()"
                        class="px-5 py-2 text-sm font-medium rounded-lg shadow-sm transition 
               bg-blue-600 text-white hover:bg-blue-700">
                        View Offers (<span x-text="selectedJob ? selectedJob.number_of_offers : 0"></span>)
                    </button>

                    <a :href="'userPostJob.php?job_id=' + selectedJob.id"
                        class="px-5 py-2 text-sm font-medium rounded-lg shadow-sm transition 
            ...
            <div x-show="showCancelModal"
                ...
                <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                    <h2 class="text-lg font-semibold text-gray-800 mb-4">Confirm Cancellation</h2>
                    <p class="text-gray-600 mb-4">Are you sure you want to cancel this job? This action cannot be undone.</p>
                    <div class="flex justify-end space-x-3">
                        <button @click="showCancelModal = false"
                            class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 transition">
                            No, Keep Job
                        </button>
                        <button @click="console.log('Job ID:', selectedJob.id); cancelJob(selectedJob.id);"
                            class="px-3 py-1 sm:px-4 sm:py-2 bg-red-500 text-white font-semibold rounded-lg shadow-md hover:bg-red-700 transition">
                            Cancel
                        </button>
            ...
            <div x-show="showSuccessModal"
                ...
                x-transition>
                <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                    <h2 class="text-lg font-semibold text-gray-800 mb-4">Job Cancelled</h2>
                    <p class="text-gray-600 mb-4">The job has been successfully cancelled.</p>
                    <div class="flex justify-center">
                        <button @click="showSuccessModal = false; location.reload();"
                            class="px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition">
                            OK
                        </button>
                    </div>
                </div>
            ...
            <div x-data="jobTable()" x-init="
                                        ...
                                        <td class="border p-2 sm:p-3">
                                            <div class="flex justify-center gap-2">
                                                ...
                                                <button @click="confirmCompletion(job)"
                                                    class="px-2 py-1 sm:py-2 text-xs sm:text-sm rounded bg-green-500 text-white hover:bg-green-600 transition flex items-center w-auto">
                                                ...
                                                <button @click="openChat(job.id, job.assigned_to_id, job.assigned_to)"
                                                    class="px-2 py-1 sm:py-2 text-xs sm:text-sm rounded bg-gray-200 hover:bg-gray-300 transition flex items-center w-auto">
                                                    <img src="../img/chat-my-job.svg" alt="Chat Icon" class="w-4 h-4 sm:w-5 sm:h-5">
                                                </button>

                                                <!-- Cancel Button (Fixed) -->
                                                <button @click="showCancelConfirmation(job)"
                                                    class="px-2 py-1 sm:py-2 text-xs sm:text-sm rounded bg-red-500 text-white hover:bg-red-600 transition flex items-center w-auto">
                                                    <img src="../img/canceljob.svg" alt="Cancel Icon" class="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2">
                                                </button>
                ...
                <div x-show="showCancelConfirmModal" x-transition x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-50">
                    <div class="bg-white p-4 rounded-lg w-1/3">
                        <h3 class="text-xl font-semibold mb-4">Are you sure you want to cancel this job?</h3>
                        <div class="flex justify-end gap-2">
                            <button @click="showCancelConfirmModal = false" class="px-4 py-2 bg-gray-500 text-white rounded-md">Cancel</button>
                            <button @click="confirmCancel()" class="px-4 py-2 bg-red-500 text-white rounded-md">Confirm Cancellation</button>
                        </div>
                    </div>
                </div>

                <!-- Success Modal (Fixed) -->
                <div x-show="showCancelSuccessModal" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-50">
                    <div class="bg-white p-4 rounded-lg w-1/3">
                        <h3 class="text-xl font-semibold mb-4">Job Canceled Successfully!</h3>
                        <div class="flex justify-end">
                            <button @click="showCancelSuccessModal = false" class="px-4 py-2 bg-green-500 text-white rounded-md">Close</button>
                        </div>
                    </div>
                </div>
                ...
                <!-- Confirmation Modal -->
                <div x-show="showCompleteConfirmModal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 p-4 z-50">
                    <div class="bg-white p-4 sm:p-6 rounded shadow-lg w-full max-w-xs sm:max-w-sm">
                        ...
                        <div class="mt-4 flex justify-end space-x-2">
                            <button @click="showCompleteConfirmModal = false" class="px-3 sm:px-4 py-1 sm:py-2 bg-gray-400 text-white rounded text-xs sm:text-sm">Cancel</button>
                ...
                <!-- Cancelled Jobs Section (Only Visible for Employers in Cancelled Tab) -->
                <div x-show="isEmployer && currentTab === 'cancelled'"
                    x-data="cancelledJobs()"
                    x-init="init()"
                    class="bg-gradient-to-br from-gray-100 to-gray-200 p-4 sm:p-6 rounded-lg shadow-md">

                    <h2 class="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-gray-800 text-center">Cancelled Jobs</h2>

                    <template x-if="loading">
                        <p class="text-center text-gray-500 text-base sm:text-lg">Loading cancelled jobs...</p>
                    </template>

                    <template x-if="!loading && cancelled_jobs.length === 0">
                        <p class="text-center text-gray-500 text-base sm:text-lg">No cancelled jobs yet.</p>
                    </template>

                    <div>

                        <!-- Jobs Cancelled Before Assigning a Worker -->
                        <h3 class="text-md font-semibold text-gray-700 mt-4">Cancelled Job Posts</h3>
                        <template x-for="job in cancelled_jobs.filter(j => !j.chosen_worker_id)" :key="job.id">
                            <div class="p-4 border rounded-lg bg-gray-100">
                                <h4 class="font-semibold text-gray-800" x-text="job.job_title"></h4>
                                <p class="text-sm text-gray-600">Cancelled on <span x-text="job.cancelled_at"></span></p>
                                <p class="text-sm text-gray-600">Cancelled by: <span x-text="job.canceller_first_name + ' ' + job.canceller_last_name"></span></p>
                                <span class="bg-red-100 text-red-700 px-3 py-1 rounded-lg">Cancelled Before Assignment</span>
                            </div>
                        </template>

                        <!-- Jobs Cancelled After Assigning a Worker -->
                        <h3 class="text-md font-semibold text-gray-700 mt-4">Cancelled In-Progress Jobs</h3>
                        <template x-for="job in cancelled_jobs.filter(j => j.chosen_worker_id)" :key="job.id">
                            <div class="p-4 border rounded-lg bg-gray-100">
                                <h4 class="font-semibold text-gray-800" x-text="job.job_title"></h4>
                                <p class="text-sm text-gray-600">Cancelled on <span x-text="job.cancelled_at"></span></p>
                                <p class="text-sm text-gray-600">Cancelled by: <span x-text="job.canceller_first_name + ' ' + job.canceller_last_name"></span></p>
                                <p class="text-sm text-gray-600">Assigned Worker: <span x-text="job.worker_first_name + ' ' + job.worker_last_name"></span></p>
                                <span class="bg-red-200 text-red-800 px-3 py-1 rounded-lg">Cancelled In-Progress</span>
                            </div>
                        </template>
                    </div>
                </div>
