...
$sql = "
    SELECT 
        ...
        offers.completion_time,
        offers.creation_time,
        CONCAT(user.first_name, ' ', user.last_name) AS employer_name  -- ✅ Employer Full Name
    FROM offers
    JOIN jobs ON offers.job_id = jobs.id
    JOIN user ON jobs.user_id = user.id  -- ✅ Join with User Table
...
$sql = "
    SELECT 
        ...
        offers.offer_amount,
        offers.completion_time,
        cj.completed_at,  --  Ensure this column exists in your DB
        cj.rating,
        cj.review,
    ...
    JOIN user ON cj.user_id = user.id  --  Get Employer details
    WHERE cj.provider_id = ?  --  Only fetch jobs completed by the logged-in provider
...
<script>
    ...
    console.log("Submitting review for job:", this.selectedJob.id);
    console.log("Rating:", this.reviewRating, "Review:", this.reviewText);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            alert('jobid:' +this.selectedJob.id + ' ' + 'jobid:rating' +this.reviewRating + ' ' + 'review:' + this.reviewText)
            fetch('update_completed_job_review.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    job_id: this.selectedJob.id,
...
<body class="bg-gray-100" x-data="jobDashboard()" x-init="init()">
    <div class="w-full max-w-5xl mx-auto p-3 sm:p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 border-t border-gray-300 pt-4">
                <template x-for="offer in offers" :key="offer.offer_id">
                    <div class="bg-gray-50 p-3 sm:p-4 rounded-lg border border-gray-300 shadow hover:shadow-lg transition cursor-pointer"
                        ...
                        <p class="text-base sm:text-lg font-semibold text-gray-700 flex items-center">
                            <img src="../img/user-my-job.svg" alt="User Icon" class="w-4 h-4 sm:w-5 sm:h-5 mr-2">
        ...
        <div x-data="jobTable()" x-init="fetchJobs()">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
                                <div class="flex-1">
                                    ...
                                    <div class="mt-2 sm:mt-3 text-xs sm:text-sm">
                                        <span x-text="job.rating ? job.rating + ' ⭐ - ' + job.review : 'No Review'"></span>
                                        <button x-show="!job.rating" @click="openReviewModal(job)"
                                            class="ml-2 px-2 sm:px-3 py-0.5 sm:py-1 text-xs bg-green-500 text-white rounded-full hover:bg-green-600 transition">
                                            ✍️ Add Review
                                        </button>
                                    </div>
                                </div>
